...
def upload_audio_file(file_path):
    """
    Tải lên một file âm thanh và dự đoán cảm xúc.
    
    Args:
        file_path (str): Đường dẫn tới file âm thanh để phân tích.
    
    Returns:
        dict: Dictionary chứa kết quả dự đoán cảm xúc.
    """
    va = TestVoiceAnalyzer()
    result = va.predict_emotion(file_path)
    return result

def run_test(mode="record", file_path=None):
    """
    Chạy thử nghiệm phân tích giọng nói.
    
    Args:
        mode (str): Chế độ chạy, "record" để ghi âm mới hoặc "upload" để tải file lên.
        file_path (str, optional): Đường dẫn tới file âm thanh nếu mode là "upload".
    """
    va = TestVoiceAnalyzer()
    
    if mode == "record":
        print("Chế độ ghi âm trực tiếp:")
        path, _ = va.record_audio(duration=3.34)
        result = va.predict_emotion(path)
    elif mode == "upload" and file_path:
        print(f"Phân tích file âm thanh: {file_path}")
        result = va.predict_emotion(file_path)
    else:
        result = {"Lỗi": "Vui lòng chọn chế độ hợp lệ và cung cấp file âm thanh (nếu cần)"}
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "record":
            ...
            # Chạy chế độ upload với đường dẫn file được chỉ định
            run_test("upload", sys.argv[2])
        else:
            print("Cách sử dụng: python test_emo_voice.py [record|upload <file_path>]")
            print("Vui lòng nhập đường dẫn file âm thanh:")
            file_path = input().strip()
            if file_path:
                run_test("upload", file_path)
    else:
        # Mặc định chạy chế độ upload nếu không có tham số
        print("Vui lòng nhập đường dẫn file âm thanh:")
        file_path = input().strip()
        if file_path:
            run_test("upload", file_path)
        else:
            print("Không có đường dẫn file. Chuyển sang chế độ ghi âm...")
            run_test("record")
